...
import { PasswordUpdateDto } from '../../user/dto/password.dto'
...
export class AccountController {
  ...
  @AllowAnon()
  async profile(@AuthUser() user: any): Promise<AccountInfo> {
  ...
  @AllowAnon()
  async logout(@AuthUser() user: any, @Req() req: FastifyRequest): Promise<void> {
    const accessToken = (req.headers.authorization as string)?.replace('Bearer ', '') || '';
    await this.authService.clearLoginStatus(user, accessToken);
  ...
  @AllowAnon()
  async permissions(@AuthUser() user: any): Promise<string[]> {
